...
define_hls_module B_search ../B_search.cpp

### 4. Define your HLS configuration (arbitrary names, BASIC and DPA in this example). 
define_hls_config B_search BASIC
define_hls_config B_search DPA       --dpopt_auto=op,expr
define_hls_config B_search PIPELINE  -DII=8
define_hls_config	B_search UNROLL
...
define_sim_config V_BASIC "B_search RTL_V BASIC" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.3 The Verilog simulation for HLS config "DPA". 
define_sim_config V_DPA "B_search RTL_V DPA" -argv "$IN_FILE_NAME $OUT_FILE_NAME"

define_sim_config V_PIPELINE "B_search RTL_V PIPELINE" -argv "$IN_FILE_NAME $OUT_FILE_NAME"

define_sim_config V_UNROLL "B_search RTL_V UNROLL" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
